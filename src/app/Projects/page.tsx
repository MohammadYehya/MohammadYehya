...
import { MyProjects } from "@/lib/data/config";
...
export default function Projects() {
  ...
  return (
    <>
      <Navbar />
      <div className={`my-10 w-full flex justify-center transition-all duration-1000 ${open ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <div className="flex flex-wrap justify-center gap-y-2 md:gap-x-2">
            {MyProjects.content.map((item, index) => {
              return (
                <Card
                  key={index}
                  className="flex md:flex-1 flex-col w-full md:min-w-[300px] md:max-w-[300px] p-5 hover:scale-110 hover:cursor-pointer transition-all duration-500"
                  onClick={() => {
                ...
                >
                  ...
                  <div className="flex flex-1 flex-wrap gap-2">
                    {item.tech.map((item, index) => (
                      <div
                        className="w-10 h-10 border border-black rounded-md"
                        key={index}
                      ></div>
                    ))}
                  </div>
